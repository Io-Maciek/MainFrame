...
impl<'a> Message<'a> {
    pub fn get_from_flash(flash: Option<FlashMessage<'_>>) -> Option<Message> {
        if let Some(_f) = flash {
            ...
            println!("{:?}", &fl.0);
            if &fl.0 == "error" {
                Some(Message { color: "danger", text: fl.1, optional: None })
            } else if &fl.0 == "success" {
                Some(Message { color: "success", text: fl.1, optional: None })
            } else if &fl.0 == "error_w_nick"{
                println!("asdadasdsadsad");
                let splitted = fl.1.split(";;;").collect::<Vec<&str>>();
                Some(Message { color: "danger", text: splitted[0].into(), optional: Some(splitted[1].into()) })
            }
            else {
                None
            }
...
#[get("/register")]
async fn register_new(jar: &CookieJar<'_>, mut db: Connection<SQL>, flash: Option<FlashMessage<'_>>) -> Result<Template, Redirect> {
    match User::get_from_cookies(&mut *db, &jar.clone()).await {
        None => {
            ...
            Ok(Template::render("rejestracja", context! {
					title: "MainFrame",
...
async fn register_new_post(jar: &CookieJar<'_>, mut db: Connection<SQL>, maker_user: Form<UserMaker<'_>>) -> Flash<Redirect> {
    ...
    match maker_user.into_inner().create_user() {
        Ok(user) => {
            match user.insert(&mut *db).await {
                ...
                Err(_) => Flash::error(Redirect::to(uri!(register_new)), format!("Nick <strong>{}</strong> jest już zajęty!", u))
            }
        }
        Err(err) => Flash::new(Redirect::to(uri!(register_new)), "error_w_nick",format!("{err};;;{u}"))
    }
}
...
fn rocket() -> Rocket<Build> {
    ...
    rocket::custom(figment)
        ...
        .mount("/", routes![register_new_post,register_new,font_awesome,delete_sharing, add_new_sharing_user, favicon,
			index,index_login,index_logout,get_file_by_id,send_file,delete_file,change_filename])
